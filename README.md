...
# Setting Everything Up ⚙
### Modify your `.env`:
- If you just cloned / pulled this template on github you have to manually create a `.env` file within the root directory of the codebase, if your confused about this root directory just create a file named `.env` where `.env.example` exist;
- Copy the variables within `.env.example`, paste it within the now created `.env` file and overwrite it the configurations. **Note:** that the codebase is heavily reliant on the variables within `.env` file and it may not work if there's incorrect value assinged.
- If you're wondering why you have to manually create the `.env` file, that's because it wasn't part of the files I pushed on github, and that reason is because of `.gitignore`,
- This `.gitignore` is also another file that Git reads, and all files registered here are not beeing part of the version control or not being shared, which is good for `.env` files because it contains system specific configurations such as API keys.

### Run your composer commands
- This command installs all the packages registered within `composer.json`, ensuring that your codebase will contain all the packages that this template is using:
```php
// teminal
composer intall
```
...
# Composer 🎼
### What is Composer?
- Composer is tool for maanging PHP packages and dependencies.
- Without composer, you'd have to manually download ZIP packages, extract them and manually place them on your project,
- but with the composer, you can `pull` these packages using your terminal and they will autoamtically be placed together with their dependecies to your codebase.

### Composer Autoload
- Before we are making use of PHP method named `spl_autoload_register` to autoload all of our files:
```php
// public > index.php
...
```
- But since this autoloader only knows the namespaces of the packages that we `pull` using compsoer, we have to manually add our own namespace to it.
- Inside `compose.json`, you have to manually add these:
```php
// composer.json
{
    "autoload": {
...
```
- These introduces composer to the classes that we have inside `App` and `Core` namespaces.
- Now to register these namespace we have to run this on the terminal:
```php
# for development (autoload only the needed)
composer dump-autoload

# for production ( -optimized / it registers many other classes and takes time to finish)
composer dump-autoload -o
```
- You can then go to your `autoload_psr4.php`, and check if the namepaces you just registered is there:
```php
